...
### 目标
用最小 DSL 表达算子设计意图，便于 LLM 理解和 triton-op-coding 实现。

### 原则
- **极简原语**：只有少数核心操作（alloc/load/store/compute/...）
- **统一语法**：所有操作都是函数调用风格，无语法差异
- **标准控制流**：使用 Python for/range 语法
...
`@llm_hint` 给 LLM 提供优化提示，帮助 triton-op-coding 选择最优实现策略。
...
**语义化描述，由 triton-op-coding 根据硬件文档选择具体实现。**

#### 存储要求（性能层次）

| hint | 含义 | Ascend NPU 映射 |
|------|------|-----------------|
| `"fastest"` | 最快访问速度，容量小 | L0（L0A/L0B/L0C） |
| `"fast"` | 快速访问，中等容量 | L1 Buffer / UB |
| `"medium"` | 中等速度，较大容量 | L2 Cache |
| `"slow"` | 较慢但容量大 | Global Memory (HBM) |
...
# triton-op-coding 会根据 Ascend NPU 硬件文档将其映射为：
# fastest → L0C（Cube 累加器）
# fast → L1 Buffer / UB（Unified Buffer）
```
...
X[start:end]                   # 一维切片
A[i_outer:i_outer+m0, k_outer:k_outer+k0]  # 参数化切片
```
...
reduce_sum(src, axis, dst)    # dst = sum(src, axis=axis)，axis 可为 list（多轴同时 reduce）
...
sketch relu {
  ...
  for i in range(0, ceil(N, tile_size)):
    x_tile = alloc([tile_size], llm_hint=["fast", "input_cache"])
    y_tile = alloc([tile_size], llm_hint=["fast", "output_buffer"])
...
sketch softmax {
  ...
  @llm_hint("parallel")
  for b in range(B):
    x_row = alloc([N], llm_hint=["fast", "input_cache"])
    y_row = alloc([N], llm_hint=["fast", "output_buffer"])
    max_val = alloc([1], llm_hint=["fastest", "accumulator"])
    sum_val = alloc([1], llm_hint=["fastest", "accumulator"])
...
2. **再加内存管理**：alloc 合适的 tile
3. **然后加数据流**：load → compute → store
4. **最后加优化 hint**：@llm_hint 装饰器

### Tile 大小设置
- 考虑 Ascend NPU UB 容量（参考硬件规格文件）
- 优先选择 2 的幂次（128, 256, 512, 1024）
- 保证 256B / 512B 数据对齐要求

### 错误避免
- **不要混用抽象层次**：要么用高级函数（gelu），要么用基础运算（add+mul）
- **明确数据流向**：每个 load 都要有对应的 compute，每个 compute 都要有对应的 store
- **合理使用 hint**：不要过度优化，先保证逻辑正确
