...
Sandbox image construction assets are maintained locally and are not included in this example PR. Supply a prepared image containing Claude Code, Python, Ascend/CANN, torch-npu, Triton Ascend, sudo and the `claude` user. It must satisfy the paths and verifier/lease interfaces in `task_config_kernel_bench.yaml`.

If you have the local sandbox construction assets, build the image on each remote Docker daemon:
...
- Triton/NPU skills: [CANNBot Skills PR #205](https://gitcode.com/cann/cannbot-skills/pull/205). This identifies the supplied skills' provenance, not a guarantee that the PR alone provides a complete recipe image.
...
Record the source revisions used for experiments. These links do not replace the applicable licences or redistribute the dataset or skills in this PR.
...
Adjust model parallelism and concurrency to the actual cluster. A 32768-token training limit is a smaller bring-up setting, not a guarantee of sufficient memory.
...
The postprocessor selects best-prefix trajectories at assistant boundaries. Multiple chains fall back to `all_final`, since a scalar assistant index cannot identify a Gateway chain. Missing implementations are not filtered by default (`drop_no_impl=False`); retry is disabled by default. Other trajectory validity and length constraints still apply. Token/mask/logprob alignment and framework reward fields are preserved.
...
Early-stop markers are terminal for an attempt: subsequent tool admissions are denied, and late verifier completions cannot clear them. This is cooperative admission control, not a hard kill of an in-flight tool or model request.
...
The recipe-specific CPU tests are retained locally, not included in this PR. With that local test directory available, run:

```bash
python -m pytest -q tests/uni_agent/tasks/kernel_bench
