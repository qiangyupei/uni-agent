...
def _crop_routed_experts(trajectory: Trajectory, response_end: int):
    """Crop R3 routes in absolute input-token coordinates.

    ``routed_experts`` is aligned to ``prompt_ids + response_ids`` rather than
    to model-output tokens only, so a response prefix keeps the prompt rows too.
    The generic framework remains responsible for its existing terminal-row
    alignment behavior.
    """
    ...
    if routed_experts is None:
        return None
    prefix_len = len(trajectory.prompt_ids) + response_end
    prefix = routed_experts[:prefix_len]
...
def _select_best(
    ...
    *,
    ...
    if best_index >= len(spans):
        return None
    response_end = spans[best_index][1]
    if not _within_limit(trajectory, response_end, max_total_tokens):
        raise ValueError("best-assistant prefix exceeds max_total_tokens; cannot assign its reward to an earlier prefix")
    return [crop_to_assistant_prefix(trajectory, response_end, reason="best_assistant")]
